  ...
  <div class="app-container">
    ...
    <transition name="el-zoom-in-top">
      <div class="filter-container" v-show="showSearchMore">
        ...
        <el-input placeholder="调出单编号" v-model.trim="listQuery.parentBillCode" style="width: 200px;" class="filter-item" @keyup.enter.native="getList"/>
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_channel_qddr_add'" :list-loading.sync="listLoading" bill-type="channel2channel_in"/>
      <edit-button v-permission="'bill_channel_qddr_edit'" :list-loading.sync="listLoading" bill-type="channel2channel_in" :selected-ids="selectedIds"/>
      <el-button v-permission="'bill_channel_qddr_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_channel_qddr_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED').length > 0">反审核</el-button>
      <delete-button v-permission="'bill_channel_qddr_delete'" :list-loading.sync="listLoading" bill-type="channel2channel_in" :selected-ids="selectedIds" @get-list="getList"/>
      ...
      <el-button v-permission="'bill_channel_qddr_print'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-printer" @click="printDialogVisible = true" :disabled="listLoading || selectedIds.length !=  1">打印</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('parent_code') > -1" label="调出单编号" align="center">
        <template slot-scope="scope">
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="channel2channel_in" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="channel2channel_in" :selected-ids="selectedIds" @get-list="getList"/>

    <export-dialog :show.sync="exportDialogVisible" bill-type="channel2channel_in"/>

    <import-dialog :show.sync="importDialogVisible" bill-type="channel2channel_in" bill-key="c2cin" @get-list="getList"/>

    <print-dialog :show.sync="printDialogVisible" :selected-ids="selectedIds" bill-type="channel2ChannelIn"/>

  </div>
</template>

<script>
  import {getList} from '@/api/bill/channel2channelIn'
