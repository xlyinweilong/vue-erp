            ...
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <el-form-item label="调出渠道" prop="channelId">
                  <el-select :disabled="isDetail" v-model="form.channelId" filterable clearable remote default-first-option placeholder="请输入调出渠道编号" :loading="loadingOptionChannelList" style="width: 100%" :remote-method="searchChannelOption">
              ...
              <el-col :span="6">
                <el-form-item label="调入渠道" prop="toChannelId">
                  <el-select :disabled="isDetail" v-model="form.toChannelId" filterable clearable remote default-first-option placeholder="请输入调入渠道编号" :loading="loadingOptionToChannelList" style="width: 100%" :remote-method="searchToChannelOption">
                    <i slot="prefix" class="el-input__icon el-icon-search"></i>
...
<script>
  ...
  import {save, get} from '@/api/bill/channel2channelOut'
          ...
          if (valid) {
            ...
            save(this.form).then(response => {
              ...
              this.$store.dispatch('delView', thisView).then(() => {
                let backView = this.$store.state.tagsView.visitedViews.find(r => r.fullPath == "/bill/channel/channel2channel_out")
                if (backView != null) {
      ...
      searchToChannelOption(query) {
        if (query !== '') {
          ...
          getChannelList({pageIndex: 1, pageSize: 10, code: query}).then(response => {
            ...
            this.optionToChannelList = response.data.content
          })
        } else {
