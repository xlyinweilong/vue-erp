<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_channel_qddc_add'" :list-loading.sync="listLoading" bill-type="channel2channel_out"/>
      <edit-button v-permission="'bill_channel_qddc_edit'" :list-loading.sync="listLoading" bill-type="channel2channel_out" :selected-ids="selectedIds"/>
      <el-button v-permission="'bill_channel_qddc_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_channel_qddc_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED').length > 0">反审核</el-button>
      <delete-button v-permission="'bill_channel_qddc_delete'" :list-loading.sync="listLoading" bill-type="channel2channel_out" :selected-ids="selectedIds" @get-list="getList"/>
      <el-button v-permission="'bill_channel_qddc_import'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-upload2" @click="importDialogVisible = true" :disabled="listLoading">导入</el-button>
      <el-button v-permission="'bill_channel_qddc_export'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-download" @click="exportDialogVisible = true" :disabled="listLoading || total ==  0">导出</el-button>
      <el-button v-permission="'bill_channel_qddc_print'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-printer" @click="printDialogVisible = true" :disabled="listLoading || selectedIds.length !=  1">打印</el-button>
    </div>
    <el-table
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="channel2channel_out" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="channel2channel_out" :selected-ids="selectedIds" @get-list="getList"/>
    ...
    <print-dialog :show.sync="printDialogVisible" :selected-ids="selectedIds" bill-type="channel2ChannelOut"/>
  </div>
</template>
