<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_channel_qdsy_add'" :list-loading.sync="listLoading" bill-type="channel_loss"/>
      <edit-button v-permission="'bill_channel_qdsy_edit'" :list-loading.sync="listLoading" bill-type="channel_loss" :selected-ids="selectedIds"/>
      <el-button v-permission="'bill_channel_qdsy_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_channel_qdsy_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED').length > 0">反审核</el-button>
      <delete-button v-permission="'bill_channel_qdsy_delete'" :list-loading.sync="listLoading" bill-type="channel_loss" :selected-ids="selectedIds" @get-list="getList"/>
      <el-button v-permission="'bill_channel_qdsy_import'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-upload2" @click="importDialogVisible = true" :disabled="listLoading">导入</el-button>
      <el-button v-permission="'bill_channel_qdsy_export'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-download" @click="exportDialogVisible = true" :disabled="listLoading || total ==  0">导出</el-button>
      <el-button v-permission="'bill_channel_qdsy_print'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-printer" @click="printDialogVisible = true" :disabled="listLoading || selectedIds.length !=  1">打印</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('bill_code') > -1" label="单据编号" align="center" width="300%">
        <template slot-scope="scope">
          <router-link :to="'/bill/channel/channel_loss_detail/'+scope.row.id" class="link-type">
            {{ scope.row.code }}
          </router-link>
        </template>
      </el-table-column>
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="channel_loss" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="channel_loss" :selected-ids="selectedIds" @get-list="getList"/>

    <export-dialog :show.sync="exportDialogVisible" bill-type="channel_loss"/>

    <import-dialog :show.sync="importDialogVisible" bill-type="channel_loss" bill-key="ci" @get-list="getList"/>

    <print-dialog :show.sync="printDialogVisible" :selected-ids="selectedIds" bill-type="channelLoss"/>

  </div>
</template>

<script>
  import {getList} from '@/api/bill/channelLoss'
