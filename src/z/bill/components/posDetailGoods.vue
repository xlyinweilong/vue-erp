  ...
  <div>
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
          <input-goods1 ref="inputGoods1" v-show="inputType === 1" @add-goods="addGoods" />
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-row :gutter="20">
      ...
      >
        ...
        <el-table-column label="颜色" align="center" min-width="250">
          <template slot-scope="scope">
            <span>{{scope.row.goodsCode}}</span>
          </template>
        </el-table-column>
        <el-table-column label="尺码" align="center" min-width="250">
          <template slot-scope="scope">
            <span>{{scope.row.goodsCode}}</span>
          </template>
        </el-table-column>
    ...
    <pagination v-if="goodsList.length != null" :total="goodsList.length" :page.sync="listQuery.pageIndex" :limit.sync="listQuery.pageSize"/>
...
<script>
  ...
  export default {
    computed: {
      detailList: function () {
        return this.goodsList.filter(r => this.listQuery.searchKey == ''
    ...
    methods: {
      ...
      initFocus() {
        if (this.inputType === 0) {
          ...
          this.$nextTick(() => this.$refs.inputGoods1.$refs.selectGoods.focus())
        }
      },
      //明细数据
      //扫锚
      ...
      addGoods(goods){
        ...
        if (inGoods == null) {
          ...
          this.goodsList.unshift(JSON.parse(JSON.stringify(goods)))
        } else {
