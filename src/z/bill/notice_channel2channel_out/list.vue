<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_channel_nqddc_add'" :list-loading.sync="listLoading" bill-type="notice_channel2channel_out"/>
      <edit-button v-permission="'bill_channel_nqddc_edit'" :list-loading.sync="listLoading" bill-type="notice_channel2channel_out" :selected-ids="selectedIds"/>
      <el-button v-permission="'bill_channel_nqddc_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_channel_nqddc_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED').length > 0">反审核</el-button>
    ...
    >
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('bill_code') > -1" label="单据编号" align="center" width="300%">
        <template slot-scope="scope">
          <router-link :to="'/bill/channel/notice_channel2channel_out_detail/'+scope.row.id" class="link-type">
            {{ scope.row.code }}
          </router-link>
        </template>
      </el-table-column>
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="notice_channel2channel_out" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="notice_channel2channel_out" :selected-ids="selectedIds" @get-list="getList"/>

    <export-dialog :show.sync="exportDialogVisible" bill-type="notice_channel2channel_out"/>
...
<script>
  import {getList} from '@/api/bill/noticeChannel2channelOut'
