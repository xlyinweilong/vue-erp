<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_chain_order_add'" :list-loading.sync="listLoading" bill-type="order" parent-type="chain"/>
      <edit-button v-permission="'bill_chain_order_edit'" :list-loading.sync="listLoading" bill-type="order" :selected-ids="selectedIds" parent-type="chain"/>
      <el-button v-permission="'bill_chain_order_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_chain_order_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED' || r.totalQuotedCount > 0).length > 0">反审核</el-button>
      <delete-button v-permission="'bill_chain_order_delete'" :list-loading.sync="listLoading" bill-type="order" :selected-ids="selectedIds" @get-list="getList"/>
      <el-button v-permission="'bill_chain_order_import'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-upload2" @click="importDialogVisible = true" :disabled="listLoading">导入</el-button>
      <el-button v-permission="'bill_chain_order_export'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-download" @click="exportDialogVisible = true" :disabled="listLoading || total ==  0">导出</el-button>
      <el-button v-permission="'bill_chain_order_print'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-printer" @click="printDialogVisible = true" :disabled="listLoading || selectedIds.length !=  1">打印</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('bill_code') > -1" label="单据编号" align="center" width="300%">
        <template slot-scope="scope">
          <router-link :to="'/bill/chain/order_detail/'+scope.row.id" class="link-type">
            {{ scope.row.code }}
          </router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('parent_code') > -1" label="采购单" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column v-if="diyValues.length == 0 || diyValues.indexOf('total_quote_count') > -1" label="被引用数量" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.totalQuotedCount}}</span>
        </template>
      </el-table-column>
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="order" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="order" :selected-ids="selectedIds" @get-list="getList"/>

    <export-dialog :show.sync="exportDialogVisible" bill-type="order"/>

    <import-dialog :show.sync="importDialogVisible" bill-type="order" bill-key="order" @get-list="getList"/>

    <print-dialog :show.sync="printDialogVisible" :selected-ids="selectedIds" bill-type="order"/>

  </div>
</template>

<script>
  import {getList} from '@/api/bill/order'
