<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <add-button v-permission="'bill_warehouse_ckpd_add'" :list-loading.sync="listLoading" bill-type="warehouse_inventory" parent-type="warehouse"/>
      <edit-button v-permission="'bill_warehouse_ckpd_edit'" :list-loading.sync="listLoading" bill-type="warehouse_inventory" :selected-ids="selectedIds" parent-type="warehouse"/>
      <el-button v-permission="'bill_warehouse_ckpd_audit'" class="filter-item" style="margin-left: 10px;" type="warning" icon="el-icon-goods" @click="auditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'PENDING').length > 0">审核</el-button>
      <el-button v-permission="'bill_warehouse_ckpd_unaudit'" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-sold-out" @click="unAuditDialogVisible = true" :disabled="listLoading || selectedIds.length == 0 || selectedIds.filter(r => r.status != 'AUDITED' && r.status != 'COMPLETE').length > 0">反审核</el-button>
    ...
    <audit-dialog :show.sync="auditDialogVisible" bill-type="warehouse_inventory" :selected-ids="selectedIds" @get-list="getList"/>

    <un-audit-dialog :show.sync="unAuditDialogVisible" bill-type="warehouse_inventory" :selected-ids="selectedIds" @get-list="getList"/>
...
<script>
  import {getList} from '@/api/bill/warehouseInventory'
