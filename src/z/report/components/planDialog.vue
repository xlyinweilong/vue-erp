<template>
  <el-dialog title="查询" :visible="show" width="45%" @close="onClose" @open="onOpen">
    ...
    <plan-detail-dialog :show.sync="planDetailDialogVisible" @getList="getList" :plan-id="planId" :tableType="tableType" :reportDic="reportDic"/>
  </el-dialog>
</template>
...
<script>
  ...
  import planDetailDialog from '@/z/report/components/planDetailDialog'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true
        getList({table: this.tableType}).then(response => {
