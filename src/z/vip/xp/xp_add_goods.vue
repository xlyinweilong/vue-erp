<template>
  <div class="app-container">
    ...
    <el-dialog :title="dialogStatus==='create' ? '新增' : (temp.id == null ? '加载中...':'修改')" :visible.sync="dialogFormVisible">
      <el-form ref="vipXpRuleGoodsForm" :rules="rules" :model="temp" v-loading="saving || (dialogStatus !='create' && temp.id == null)">
        <el-row :gutter="20">
          <el-col :span="12">
...
<script>
  import {getGoodsList, saveGoods, deleteGoods} from '@/api/vip/vipXpUpRule'
  ...
  export default {
    ...
    created() {
      ...
      this.listQuery.vipXpUpRuleId = id
      this.getList()
    },
    methods: {
      //获取列表
      ...
      //弹出框新增
      createElement() {
        this.temp = {id: '', vipXpUpRuleId: this.listQuery.vipXpUpRuleId, goodsBrandId: '', goodsBrandName: '', goodsCategoryId: '', goodsCategoryName: '', goodsYearId: '', goodsYearName: '', goodsSeasonId: '', goodsSeasonName: ''}
        this.dialogStatus = 'create'
        this.dialogFormVisible = true
        this.$nextTick(() => {
          this.$refs['vipXpRuleGoodsForm'].clearValidate()
        })
      ...
      saveData() {
        this.$refs['vipXpRuleGoodsForm'].validate((valid) => {
          if (valid) {
            this.saving = true
